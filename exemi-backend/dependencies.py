...
def get_canvas_provider_base_url(provider: str) -> str:
    """
    Convert a Canvas provider identifier into a base URL.

    Provider values can be:
    - A university/provider name like "swinburne" (legacy)
    - A hostname like "canvas.example.edu"
    - A full URL like "https://canvas.example.edu"
    """
    provider = provider.strip()
    if provider.startswith("http://") or provider.startswith("https://"):
        return provider.rstrip("/")
    if "." in provider:
        return f"https://{provider}".rstrip("/")
    return f"https://{provider}.instructure.com"
...
async def is_magic_valid(
    ...
    """
    providers: list[tuple[str, str | None]] = [(provider, provider_canvas_url), *fallback_providers]
    ...
    for i, (provider, candidate_canvas_url) in enumerate(providers):
        ...
        async with httpx.AsyncClient(timeout=15) as client:
            response = await client.get(f"{base_url}/api/v1/users/self", params={
            "access_token":magic,
            })
...
async def encrypt_magic(
    ...
    fallback_providers : list[tuple[str, str | None]] = [],
    university_canvas_url : str | None = None,
    ...
    actual_university_name : str | None = await is_magic_valid(
        provider=university_name,
        fallback_providers=fallback_providers,
        magic=magic,
        provider_canvas_url=university_canvas_url
    )
...
def get_fallback_providers(
    ...
    fallback_providers = [(p.name, p.canvas_url) for p in fallback_provider_objects]
